...
By the time I joined the company, EIM was a monster of a web app, boasting more than 16,000 files (in the core project repository alone) and no UX or UI design strategy to speak of.
...
I quickly learned (for more complex modules, at least) it was helpful to provide both an interactive prototype (i.e. InVision) as well as a static document (e.g. a PDF) containing all the wireframes/mockups with inline notes.  The static document provides thorough, detailed specifications, while the prototype functions as a higher-level overview of the interaction flow.
...
Whether I provided these detailed specs or only mid-fidelity wireframes, I always strived to maintain constant communication with the developer(s) throughout the process, and I personally reviewed the changes (committed into the code repository) to be sure everything came out as intended.
...
Through this effort (and alongside side efforts to identify deprecated files), the product code repository was reduced from around 16,000 files to fewer than 6,000 files—making future maintenance, platform-wide UI or tech stack updates, help documentation efforts, and overall design and development work much more manageable and less burdensome.
